...
class AMD(object):
    def __init__(self, table, params=None, abund_mult={}):
        ...
        for xiline in data:
            ...
            for ion in self.ind:
                if ion in ('ion_run', 'delta_r', 'x_e', 'n_p', 'frac_heat_error'):
                    continue
                try:
    ...
    def rollTheDice(self, component, rolls, throw=True, overtime=1000, **kwargs):
        ...
        while i < overtime and ((nh < lower).any() or (nh > upper).any()):
            ...
            i += 1
        if i == overtime:
            print("-E- Best fit outside of bounds after overtime!")
        self._rVec[c] = save
    ...
    def converge(self, component, **kwargs):
        ...
        while True:
            ...
            try:
                ...
                if cverbose:
                    print('Got:', self._last)
                kwargs['guess'] = e.new
                maxiter -= 1
                if not maxiter:
                    raise ValueError("Exceeded maximum iteration limit")
                continue
    ...
    def error(self, *indices, **kwargs):
        ...
        if not indices:
            indices = tuple(range(len(self._last.x)))
    ...
    def AMD(self, component, guess=None, plot=False, onlyElem=None,
        ...
        for ion in self._coeff[c]:
            ...
            if elem == onlyElem:
                useOnly.add(ion)
            if onlyElem is None and elem not in filterElems:
                useOnly.add(ion)
        if not useOnly:
            useOnly = None

        bounds = [(0, None) for _ in range(len(guess))]
        result = minimize(self.AMDQuality, guess, args=(c, None, add, True), bounds=bounds, method='slsqp',
        ...
        if result.success:
            NH = result.x
            if plot:
                toplot = concatenate((array(self.xilist[c][1:-1]).reshape(-1, 1),
                                     (self.dxilist[c][1:-1]/2.0).reshape(-1, 1),
    ...
    def AMDEst(self, *components):
        ...
        def xi(ion):
            return self.probableXi(e(ion))

        def nh(ion):
            return self.NH(e(ion), xi(ion), self.params[c][elem][ion])

        def nhe(ion):
            return abs(nh(ion)-self.NH(e(ion), xi(ion), self.errors[c][e(ion)]))
        ...
        Iplot.y.label(r'log ($N_\mathrm{H}$ / $10^{18}$ cm$^{-2}$)')
        Iplot.x.label('log $\\xi$')
    ...
    def NH(self, ion, xi, NI):
        elem, charge = self.getIon(ion, split=True)
        if type(charge) is int:
            charge = RomanConversion.toRoman(charge).lower()
    ...
    def probableXi(self, ion, ion2=None, frac=None, ):
        ...
        if ion2 is None:
            if frac is not None:
                raise ValueError('Fraction=ion/ion2 must be None if ion2 is None.')
    ...
    def getIon(self, ion, split=False):
        if ion not in self.ind or split is True:
            try:
                try:
                    elem, charge = ion.split('_')
                except ValueError:
