...
class ibifit(_singleModel):
        ...
        if os.path.isfile(lines):
            ...
            for elemf in glob(lines+'/*'):
                ...
                for ionf in glob(elemf+'/*'):
                    ...
                    self.ions[ion].lines = list(ibifit.getlines(linef, fcut, number=ncut))
                    self.params[ion] = 0
        else:
    ...
    def prettyParam(param):
        if param in set(('kT', 'redshift', 'vturb', 'C')):
            return param
    ...
    def getlines(linef, fcut=10**-3, edge=False, number=float('inf')):
        ...
        for line in open(linef):
            ...
            if count < title:
                continue
            line = line.split()
            if not edge and float(line[f]) < fcut:
                continue
            yield [float(line[i]) for i in ind]
            if edge:
                break
            if count-title+1 >= number:
                break
    ...
    def taumodel(self, wl, ion, kT, vturb):
        ...
        for line, rt, f in self.ions[ion].lines:
            # Width - sqrt(2)sigma
            sg = V*10**13/line  # Hz
    ...
    def generator(self):
        ...
        for ion in self.ions:
            exec(('self.ions[ion].t = lambda wl: self.taumodel(wl,"' + str(ion)+'",' + str(kT) + ',' +
                  str(vturb)+')'), locals(), globals())

    def ionExp(self, ion, wl, units):
        try:
            return -self.params[ion]*units*self.ions[ion].t(wl)
        except TypeError:
            raise Exception('No data found for '+ion+'!')
        ...
        for param in self.changed:
            if param in gparams:
                if not gen:
                    ...
                    gen = True
                    break
            elif param != 'redshift' and param != 'C':
                Nions.append(param)
