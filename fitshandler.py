...
class Data(fitsHandler):
    ...
    def loadText(self, fname):
        ...
        self.errors = (lambda rebin=1, _=1, row=False, x=self.errorarray:
                       Data.ndrebin(x, rebin) if row else Data.ndrebin(x, rebin).reshape(-1, 1))
...
class Events(fitsHandler):
    def __init__(self, event_file):
        ...
        self.pixel_per_arc_second = 1/(SkyCoord(*self.wcs.wcs_pix2world(0, 1, 1), frame='fk5', unit='deg'
                                                ).separation(SkyCoord(*self.wcs.wcs_pix2world(0, 0, 1),
                                                                      frame='fk5', unit='deg')).dms.s)
    ...
    def plot(self, obj=None, coord='pixel'):
        ...
        def vals(x, y):
            return 'x=%d, y=%d, z=%s' % (x+1, y+1, self._plotval(x, y))
    ...
    def object(self, center, what='pixel', constant=None):
        ...
        while self._is_effective_radius(center, R):
            R += 1
    ...
    def background(self, center, outer_coefficient=1.25, constant=None, coord='pixel'):
        ...
        if constant is not None:
            return x, y, constant[0], constant[1]
