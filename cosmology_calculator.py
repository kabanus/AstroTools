...
def run(z,H0=69.6,WM=0.286,WV=None,verbose=0):
  if WV is None: WV = 1.0 - WM - 0.4165/(H0*H0)
...
# comoving volume computation
  ...
  if verbose == 1:
    print('For H_o = ' + '%1.1f' % H0 + ', Omega_M = ' + '%1.2f' % WM + ', Omega_vac = ', end=' ')
    ...
    print('The comoving radial distance, which goes into Hubbles law, is', end=' ')
    print('%1.1f' % DCMR_Mpc + ' Mpc or ' + '%1.1f' % DCMR_Gyr + ' Gly.')
    print('The comoving volume within redshift z is ' + '%1.1f' % V_Gpc + ' Gpc^3.')
    print('The angular size distance D_A is ' + '%1.1f' % DA_Mpc + ' Mpc or', end=' ')
  ...
  return zage_Gyr,DCMR_Mpc,DL_Mpc,kpc_DA,(5*log10(DL_Mpc*1e6)-5)
...
# or else fail
 ...
 print('%1.2f %1.2f %1.2f %1.2f %1.2f'%run(z,H0,WM,WV))
