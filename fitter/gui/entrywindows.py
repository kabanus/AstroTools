...
class ionLabeler(entryWindow):
    def __init__(self, parent, title):
        ...
        try:
            entryWindow.__init__(self, parent, '', "ioner", title)
...
class setEps(entryWindow):
    def __init__(self, parent):
        try:
            entryWindow.__init__(self, parent, '', 'epsilon', "Set forward fit step")
        except AttributeError:
            return
    ...
    def parse(self, event):
        try:
            ...
            self.parent.fitter.eps = step if step else finfo(float).eps
        except ValueError:
...
class zReader(entryWindow):
    def __init__(self, parent, data):
        try:
            entryWindow.__init__(self, parent, 'data', "ashifter", "Rest frame")
        except AttributeError:
            return
        self.data = data
...
class rebinReader(entryWindow):
    def __init__(self, parent, group=False, gui=True):
        ...
        try:
            entryWindow.__init__(self, parent, 'data', "rebinner", rebin)
        except AttributeError:
            return
...
class ignoreReader(entryWindow):
    def __init__(self, parent, what, gui=True):
        ...
        if not gui:
            ...
            return
        try:
            entryWindow.__init__(self, parent, 'data/response', "ignorer", what.title()+" x-axis values")
        except AttributeError:
            return
    ...
    def parse(self, event):
        ...
        while i < len(deleted):
            start = deleted[i]
            i += 1
            while i < len(deleted) and deleted[i] - deleted[i-1] == 1:
                i += 1
    ...
    def setUnits(self, start, end=''):
        if end != '':
            ...
            if self.parent.fitter.ptype == 1:
                ...
                start = tmp
            if self.parent.fitter.ptype == 2:
                end = '-%.2f' % self.parent.fitter.resp.wl(end)

        if self.parent.fitter.ptype == 0:
            return ('%d' % start)+end
        if self.parent.fitter.ptype == 1:
            return ('%.2f' % self.parent.fitter.resp.energy(start))+end
        if self.parent.fitter.ptype == 2:
            return ('%.2f' % self.parent.fitter.resp.wl(start))+end


class Save(entryWindow):
    def __init__(self, parent, saver=Iplot.export, title='Save (extension determines type)', default_ext='ps'):
        try:
                if saver == Iplot.export:
                    pass
                elif saver == parent.saveParams and parent.fitter.current is None:
                    ...
                    try:
                        ...
                        if parent.fitter.current is None:
        ...
        try:
            entryWindow.__init__(self, parent, '', "saver", title, 30)
        except AttributeError:
            return
        self.saver = saver
...
class paramReader(entryWindow):
    def __init__(self, parent, function, parent_field, title, multiple=False, exactToggle=False):
        try:
            entryWindow.__init__(self, parent, 'model', parent_field, title)
        except AttributeError:
            return
        if exactToggle:
            self.exact = IntVar()
...
class rangeReader(simpleWindow):
    def __init__(self, parent):
        try:
            simpleWindow.__init__(self, parent, 'model', "mranger", "Range in keV")
        except AttributeError:
            return
...
class zoomReader(simpleWindow):
    def __init__(self, parent):
        try:
            simpleWindow.__init__(self, parent, 'data', "zoomer", "Zoom")
        except AttributeError:
            return
