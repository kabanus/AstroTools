...
class badPlotType(Exception):
    pass


class badZoomRange(Exception):
    pass
...
def _labelaxes(self, model):
    add = ''
    if model is not None or self.area.any():
        add += ' cm$^{-2}$'
    ...
    if self.ptype == self.WAVE:
        Iplot.x.label(r'$\AA$')
        if model is None or len(model[0]) == 2:
            Iplot.y.label(r'ph s$^{-1}$ $\AA^{-1}$'+add)


def plotModel(self, start=None, stop=None, delta=None):
    if start is None:
        start = self.plotmodel[0][0]
    if stop is None:
        stop = self.plotmodel[-1][0]
    if delta is None:
        delta = self.plotmodel[1][0]-self.plotmodel[0][0]
    energies = array(
...
def _plotOrSave(save=None, model=None, data=None):
    if save is None:
        if data is not None:
            ...
            Iplot.plotCurves(data, scatter=True, plotype=plotype, marker="+", lw=2)
...
def plot(self, save=None, user=True, keepannotations=False):
    ...
    if model is None:
        ...
        if len(model[0]) > 2:
            if self.ptype == self.WAVE:
                model = list(self.resp.wl(model, True))
            if self.ptype == self.ENERGY:
                model = list(self.resp.energy(model, True))
        else:
            if self.ptype == self.CHANNEL:
    ...
    if not keepannotations and self.labelions > 0:
        ...
        while True:
            ...
            for label in self.ionlabs:
                if label[-1] > self.labelions:
                    continue
                if label[self.ptype] > stop:
                    break
                if label[self.ptype] < start or label[3] < 0:
                    continue
                xindex = label[3]//(self.binfactor*self.data.grouping)
                if plots[0][xindex][yindex+1] == float('inf'):
                    continue
                yoffset = plots[0][xindex][yindex+1]
                if yoffset*(offsetfac+1)+plots[0][xindex][yindex] >= ymax:
                    continue
                if totaloffset[1] < offsetfac*yoffset:
                    totaloffset = (0, offsetfac*yoffset)
